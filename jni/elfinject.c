//useage : elfinject system_server
...
const char * logfilepath = "/mnt/sdcard/elf.log";
...
int inject_remote_process(pid_t target_pid, const char *library_path,const char * cydia_path,
	...
	libaddr = get_module_base(target_pid,library_path);
	...
	if (ptrace_attach(target_pid) == -1) {
		DEBUG_PRINT("can't ptrace_attach target_pid");
		if (initlog()) {
			fprintf(log_file, "[+] can't ptrace_attach target_pid %s ",
					strerror(errno));
			fclose(log_file);
		}
		goto exit;
	...
	//��ȡ��ǰ�Ĵ���������״̬
	if (ptrace_getregs(target_pid, &regs) == -1) {
		DEBUG_PRINT("can't ptrace_getregs target_pid");
		if (initlog()) {
			fprintf(log_file, "[+] can't ptrace_getregs target_pid %s ",
					strerror(errno));
			fclose(log_file);
		}
		goto exit;
	...
	if (ptrace_call_wrapper(target_pid, "mmap", mmap_addr, parameters, 6, &regs)
			== -1) {
		DEBUG_PRINT("can't call mmap in target_pid");
		if (initlog()) {
			fprintf(log_file, "[+] can't call mmap in target_pid %s ",
					strerror(errno));
			fclose(log_file);
		}
		goto exit;
...
>>>>>>> origin/master
	...
	DEBUG_PRINT( "[+] Get sohandle: %x\n", sohandle);

#define FUNCTION_NAME_ADDR_OFFSET       0x100
	//��Ҫ���õĺ����ַ���д�����ٵ��ڴ浱��ȥ ���ﶨ����һ��ƫ�� 100
	...
	if (ptrace_call_wrapper(target_pid, "dlsym", dlsym_addr, parameters, 2,
			&regs) == -1) {
		DEBUG_PRINT("can't call dlsym in target_pid");
		if (initlog()) {
			fprintf(log_file, "[+] can't call dlsym in target_pid %s ",
					strerror(errno));
			fclose(log_file);
		}
		goto exit;
...
#define FUNCTION_PARAM_ADDR_OFFSET      0x200
	...
	if (ptrace_call_wrapper(target_pid, "hook_entry", hook_entry_addr,
			parameters, 1, &regs) == -1) {
		DEBUG_PRINT("can't call hook_entry in target_pid");
		if (initlog()) {
			fprintf(log_file, "[+] can't call hook_entry in target_pid %s ",
					strerror(errno));
			fclose(log_file);
		}
		goto exit;
	...
	if (ptrace_call_wrapper(target_pid, "dlclose", dlclose, parameters, 1,
			&regs) == -1) {
		DEBUG_PRINT("can't call dlclose in target_pid");
		if (initlog()) {
			fprintf(log_file, "[+] can't call dlclose in target_pid %s ",
					strerror(errno));
			fclose(log_file);
		}
		goto exit;
...
>>>>>>> origin/master
	...
	if (-1 == target_pid) {
		...
		if (initlog()) {
			fprintf(log_file, "[+] Can't find the process %s : %s  \n",
					process_name); //run here fuck shit
			fclose(log_file);
		}
		return -1;
	}
